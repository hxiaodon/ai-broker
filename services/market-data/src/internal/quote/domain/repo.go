...
type QuoteCacheRepo interface {
	...
	// Get retrieves a quote from the cache by symbol.
	Get(ctx context.Context, symbol string) (*Quote, error)
	// MGet retrieves quotes for multiple symbols from the cache.
	MGet(ctx context.Context, symbols []string) ([]*Quote, error)
}
