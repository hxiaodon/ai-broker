...
type WatchlistRepo interface {
	...
	FindByUserID(ctx context.Context, userID int64) ([]*WatchlistItem, error)
	...
	ExistsByUserAndSymbol(ctx context.Context, userID int64, symbol string) (bool, error)
	...
	Reorder(ctx context.Context, userID int64, symbolOrder []string) error
}
