...
AMS 不可用时的降级策略：
- 使用 Redis 中的缓存限额配置（最多30分钟旧数据），仍可执行限额校验。
- **若缓存也失效或不存在**：一律拒绝出金和入金请求(保守策略),同时触发告警。理由:KYC 状态未知时放行任何资金移动都可能让冻结账户/降级用户绕过实时校验,**违反 Rule 2 AML 强制性要求**。不存在"入金风险低于出金所以可放行"的例外。
- 此规则与 `failure-handling-matrix.md` §C5 对齐。
...
**双窗口检测算法**(以 `.claude/rules/fund-transfer-compliance.md` Rule 2 为 SSOT,任一窗口命中即触发):

```
窗口 A — 24 小时滑动窗口(intra-day 拆分)
  范围: 同一用户 + 同方向(入金/出金) + 同币种
  条件:
    1. 历史交易笔数 >= 2 (含当前请求共 3+ 笔)
    2. 每笔金额均 < $10,000 USD / HK$120,000
    3. 累计金额(含当前) >= $8,000 USD / HK$96,000   ← 保守低位
  触发: SAR 候选 + AML_STATUS = REVIEW

窗口 B — 7 日滚动窗口(FinCEN 标准,跨日拆分)
  范围: 同上 + 跨自然日聚合(按 UTC 计算用户日,按 ET 计算 CTR 归属日)
  条件:
    1. 历史交易笔数 >= 2 (含当前共 3+ 笔)
    2. 每笔金额均 < $10,000 USD
    3. 累计金额(含当前) >= $10,000 USD / HK$120,000
  触发: SAR 候选 + AML_STATUS = REVIEW

聚合维度: 跨渠道(ACH + Wire + FX-funded transfer) + 跨账户(同 SSN/HKID)
```
...
- **严格禁止通知用户**(Tipping-off 禁令,见 §4 + compliance Rule 2)
- 如同时触发 CTR(单笔/当日累计 > 阈值),CTR 照常申报,但**两个流程独立日志、独立审批、客户通信使用统一脱敏话术**
...
| L3 合规官审批 | 金额 **≥ $200,000 USD（或 HK$1,500,000）**；SAR 已触发；多次 AML 标记（30天内≥3次）；用户在内部监控名单 | 首席合规官（CCO）或其授权代理 | 8小时 |
