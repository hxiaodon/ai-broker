...
type: domain-prd
version: v1.0
updated_date: 2026-03-29T16:30+08:00
surface_prd:
  - path: ../../../mobile/docs/prd/02-kyc.md
    description: "PEP 信息采集、审核流程展示"
revisions:
  - rev: 1
    date: 2026-03-29T16:30+08:00
    author: product-manager
    summary: "新增 Non-HK PEP 分类标准（平衡方案 Level 1-3）、EDD 工作流、Admin Panel 设计"
---
>
...
## 面向 Mobile/Engineering 工程师的快速指南

**本文档定义了 AMS 的 AML（反洗钱）合规完整规格**。如果你是移动端或交易引擎工程师，你需要了解：

1. **PEP 分类的 3 个 Level**（§ 4.3）
   - Level 1：高风险，自动升级 EDD，需合规经理批准
   - Level 2：中等风险，人工评估后决定是否升级 EDD
   - Level 3：低风险，标记后正常激活，后续交易监控

2. **用户可见的影响**（User-Facing）
   - 开户时多一个"官职信息"采集步骤（如用户勾选是官员）
   - 审核时间可能延长（Level 1：5-7 天，Level 2：2-3 天）
   - 提醒文案："由于风险评估要求，审核延长至 X 个工作日"

3. **账户 API 中的标记**
   - `pep_level`：1 / 2 / 3 / null（非 PEP）
   - `edd_required`：true / false
   - `aml_risk_score`：LOW / MEDIUM / HIGH

4. **与 Surface PRD 的关系**
   - Mobile PRD-02 只定义"PEP 复选框 + 官职采集"的 UI
   - 本 Domain PRD 定义分类规则、EDD 工作流、Admin Panel 设计
   - Mobile 工程师只需关心用户输入的采集和状态展示；EDD 逻辑由后端在 Admin Panel 处理
...
### 4.3 Non-HK PEP 分类标准（平衡方案 Level 1-3）

根据 2026-03-29 的产品决策，采纳"平衡方案"（Level 1-3 分层），符合 FATF 标准和 SFC 2023 指引，在风险管控和用户体验之间取得最优平衡。
...
- **人数**：约 300-500 人（全国）
- **处理**：自动升级 EDD，需合规高管（Manager 以上）批准；审核时间 5-7 个工作日
- **财富评估**：必须核实财富来源合理性，特别关注跨境资金、礼物赠与等

**Level 2 - 人工评估**（中等风险，灵活处理）
- **范围**：省副、地级市正职、中央企业一把手、大陆证监会正副职
- **人数**：约 3,000-5,000 人（全国）
- **处理**：常规 KYC 审核后（1 个工作日），合规官员在 Admin Panel 进行人工评估（1-2 天），根据财富来源决定是否升级 EDD
- **决策标准**：
  - 财富来源是否合理？（薪资、经营、继承、投资等）
  - 交易模式是否异常？（频率、金额、对手方）
  - 是否有负面新闻？（贪腐、制裁相关报道）
  - 结果：直接激活 或 升级 EDD
...
- **范围**：市副、县级、中层国企管理
- **人数**：约 10,000-20,000 人（全国）
- **处理**：自动标记，账户正常激活（3-5 天），但交易监控阈值下调 50%
- **监控**：
  - 单笔交易 > $100K 时，自动冻结等待合规官员确认
  - 月度 > $500K 时，自动升级 Level 2 进行评估
  - 异常模式（频繁换银行、大幅增加金额）自动升级 Level 1 强制审查
...
```
用户勾选"是否为中国政府官员或国企高管" → 填写官职为"省长/副总理"等
    ↓
系统自动分类为 Level 1
    ↓
账户进入 PENDING_EDD 状态（跳过常规 KYC 人工审核加速）
    ↓
合规经理进行强化尽职调查（EDD）→ 批准或拒绝
    ↓
Admin Panel 显示在"PEP EDD 队列 - Level 1 案件"
```
...
常规 KYC 审核（1 个工作日）
    ↓
Admin Panel 在"PEP EDD 队列 - Level 2 案件"显示
    ↓
合规官员进行人工评估（检查财富、交易模式、新闻）
    ↓
决策：直接激活 或 升级 EDD 或 拒绝
```
...
用户官职为"县级官员"或系统无法识别
    ↓
自动标记为 Level 3，账户正常激活
    ↓
后续由 Fund Transfer 服务监控交易
    ↓
单笔 > $100K 或月度 > $500K 时自动告警
```
