...
#### 2.2.1 上报事件类型与 ActionType 映射

| 内部 event_type | CAT ActionType | 必填字段（额外）|
|----------------|--------------|---------------|
| ORDER_SUBMITTED | NEW | senderIMID, originatingIMID, fdidCustomer, orderType, timeInForce, tradingSession, handlingInstructions |
| ORDER_ROUTED | ROUTE | routedOrderID, destination, exchangeRoutedShares |
| ORDER_AMENDED | MODIFY | origClOrdID, newClOrdID, modifiedFields[] |
| ORDER_AMEND_REJECTED | MODIFY_REJECT | origClOrdID, newClOrdID, rejectReason |
| ORDER_CANCELLED | CANCEL | cancelType (USER\|SYSTEM\|EXPIRED), originatingCancelTimestamp |
| ORDER_CANCEL_REJECTED | CANCEL_REJECT | rejectReason |
| ORDER_FILLED / ORDER_PARTIAL_FILLED | TRADE | tradeID (=ExecID), fillQuantity, fillPrice, venue, contraSideIMID |
| ORDER_REJECTED | REJECT | rejectingParty (BROKER\|EXCHANGE), rejectReason |
| ORDER_QUEUED | NEW_HELD | queueReason (PREMARKET\|HOLIDAY\|MWCB) |
| ORDER_RELEASED_FROM_QUEUE | RELEASE_HELD | originalQueueReason |

> CAT 当前 Phase 2c (Industry Member Phase 2c) 已要求覆盖以上字段。MODIFY_REJECT / CANCEL_REJECT / NEW_HELD / RELEASE_HELD 属于 2024 后新增枚举，需要重点验证。

#### 2.2.2 完整 CAT 字段字典

| 字段 | 类型 | 必填 | 含义 | 数据来源 |
|------|------|:---:|------|---------|
| `eventTimestamp` | string (RFC3339 + μs) | 是 | 事件发生的精确时刻，e.g. `2026-05-14T09:30:00.123456Z` | `order_events.created_at`（truncate 到 μs）|
| `senderIMID` | string(4) | 是 | 本券商 IMID（向 FINRA 注册）| 配置项 `CAT.SenderIMID` |
| `originatingIMID` | string(4) | 是 | 订单首次进入 NMS 的 IMID | 自营单 = senderIMID；接入券商时 = 上游 IMID |
| `CATReporterIMID` | string(4) | 是 | 上报方 IMID | = senderIMID |
| `fdidCustomer` | string(40) | 是 | 客户匿名 ID（不上报 SSN/姓名）| `accounts.fdid`（KYC 时分配，全局唯一）|
| `orderID` | string(40) | 是 | 内部订单 ID | `orders.order_id` |
| `symbol` | string(10) | 是 | NMS Symbol | `orders.symbol` |
| `side` | enum | 是 | B / S / SS (Short Sell) / SSE (Short Sell Exempt) | `orders.side` + 卖空判定 |
| `orderType` | enum | 是 | LMT / MKT / STP / STL / TS / MOO / MOC | `orders.type` |
| `price` | string (decimal) | LMT/STL 必填 | 4 位小数 | `orders.price` |
| `stopPrice` | string (decimal) | STP/STL 必填 | 4 位小数 | `orders.stop_price` |
| `quantity` | int | 是 | 订单数量 | `orders.quantity` |
| `timeInForce` | enum | 是 | DAY / GTC / IOC / FOK / OPG / CLS / GTC_EXT / DAY_EXT | `orders.time_in_force` |
| `tradingSession` | enum | 是 | PRE / REG / POST | submitted_at + market_calendar 推导 |
| `handlingInstructions` | enum | 是 | MANUAL / ALGO / DIRECT | 由 `orders.source` 推导（IOS/ANDROID/WEB=MANUAL, API+algo_flag=ALGO, 其他=DIRECT）|
| `routedOrderID` | string(40) | ROUTE 必填 | 路由后给 venue 的订单 ID | FIX ClOrdID |
| `exchangeRoutedShares` | int | ROUTE 必填 | 路由数量 | NewOrderSingle.OrderQty - LeavesQty 已成交 |
| `destination` | string(4) | ROUTE 必填 | 目标 venue MIC（XNYS, XNAS, ARCX, EDGX, ...）| SOR decision log |
| `origClOrdID` | string(40) | MODIFY/CANCEL 必填 | 改单前 / 撤单前 ID | 41 Tag |
| `newClOrdID` | string(40) | MODIFY 必填 | 改单后 ID | OrderAmend 时生成 |
| `modifiedFields` | array<enum> | MODIFY 必填 | PRICE \| QUANTITY \| STOP_PRICE \| TIF | 改单 diff |
| `tradeID` | string(40) | TRADE 必填 | 交易所成交 ID | ExecutionReport.ExecID |
| `fillQuantity` | int | TRADE 必填 | 本次 fill 数量 | LastQty |
| `fillPrice` | string (decimal) | TRADE 必填 | 本次 fill 价格 | LastPx |
| `venue` | string(4) | TRADE 必填 | 成交 venue MIC | ExecutionReport.LastMkt |
| `contraSideIMID` | string(4) | TRADE 可选 | 对手方 IMID | ExecutionReport.ContraBroker（如 venue 提供）|
| `rejectingParty` | enum | REJECT 必填 | BROKER / EXCHANGE | reject 来源 |
| `rejectReason` | string(80) | REJECT/MODIFY_REJECT/CANCEL_REJECT 必填 | 拒绝原因（机读 code + 人读 text）| ExecutionReport.Text / 内部 reject_code |
| `cancelType` | enum | CANCEL 必填 | USER / SYSTEM / EXPIRED | 内部 cancel 来源 |
| `originatingCancelTimestamp` | string | CANCEL 必填 | 用户发起撤单的时刻（区别于交易所确认时刻）| order_events ORDER_CANCEL_REQUESTED |
| `queueReason` | enum | NEW_HELD 必填 | PRE_MARKET / LUNCH / HOLIDAY / MWCB_PAUSE / EXTENDED_HOURS_OFF | OMS queue_reason 字段 |
| `clockOffsetUs` | int | 是 | 写入时刻的本节点 NTP 漂移（μs）| chrony tracking |

> 字段命名严格遵循 CAT NMS Plan Industry Member Phase 2c JSON Schema。Camel case。

#### 2.2.3 IMID / FDID 注册与分配

**IMID (Industry Member ID)**：
- 向 FINRA CAT 注册，券商唯一 4 字母标识符（如 `BRKR`）
- 上线前必须申请，配置在 Secrets Manager（`CAT.SenderIMID`）
- 多个交易实体（如 broker-dealer + investment-adviser）可能有不同 IMID

**FDID (Firm Designated ID)**：
- 内部生成（UUID v5 with namespace = 券商 UUID + customer 唯一标识）
- KYC 完成时分配并写入 `accounts.fdid`
- **跨账户类型相同 customer 共享同一 FDID**（margin / cash / IRA / 受托）
- **不可逆**：账户删除后 FDID 必须保留 7 年用于历史查询（SEC 17a-4）
- 不上报：customer 姓名 / SSN / 出生日期 / 地址 → 这些只保留在 broker 内部，FDID 是与 CAT 的唯一映射

```go
// FDID 生成
func GenerateFDID(brokerNamespace uuid.UUID, customerUUID string) string {
    return uuid.NewSHA1(brokerNamespace, []byte(customerUUID)).String()
}
```

**FDID-Customer 关联表**（CAT Reporter 维护，向 SEC 提交）：

```sql
CREATE TABLE fdid_customer (
    fdid             VARCHAR(40) PRIMARY KEY,
    customer_uuid    VARCHAR(40) NOT NULL,    -- 内部用，不上报 CAT
    primary_name     TEXT NOT NULL,           -- 加密存储
    tax_id_encrypted BYTEA NOT NULL,
    fdid_type        VARCHAR(20),             -- INDIVIDUAL / JOINT / ENTITY
    created_at       TIMESTAMP NOT NULL,
    deleted_at       TIMESTAMP NULL,          -- 软删除，保留 7 年
    UNIQUE (customer_uuid, fdid_type)
);
```
...
```json
{"actionType":"NEW","eventTimestamp":"2026-05-14T09:30:00.123456Z","orderID":"ORD-001","CATReporterIMID":"BRKR","senderIMID":"BRKR","originatingIMID":"BRKR","fdidCustomer":"FDID-12345","symbol":"AAPL","side":"B","orderType":"LMT","price":"150.2500","quantity":100,"timeInForce":"DAY","tradingSession":"REG","handlingInstructions":"MANUAL","clockOffsetUs":12}
{"actionType":"ROUTE","eventTimestamp":"2026-05-14T09:30:00.234567Z","orderID":"ORD-001","routedOrderID":"ROUT-001","CATReporterIMID":"BRKR","destination":"XNYS","exchangeRoutedShares":100,"clockOffsetUs":15}
{"actionType":"MODIFY","eventTimestamp":"2026-05-14T09:30:05.567890Z","orderID":"ORD-001","origClOrdID":"ROUT-001","newClOrdID":"ROUT-002","modifiedFields":["PRICE"],"price":"150.5000","CATReporterIMID":"BRKR","clockOffsetUs":18}
{"actionType":"TRADE","eventTimestamp":"2026-05-14T09:30:10.345678Z","orderID":"ORD-001","tradeID":"X-NYSE-9876","CATReporterIMID":"BRKR","fillQuantity":100,"fillPrice":"150.4900","venue":"XNYS","contraSideIMID":"BRK2","clockOffsetUs":22}
```

**提交时间**：
- 截止：T+1 08:00 ET（NMS Plan 强制）
- 推荐：17:00 ET（T 日）前完成首次上传，留出 15h 错误修复窗口
- 通过 SFTP 上传到 FINRA CAT Reporter Portal

**错误类型与修复期**（CAT NMS Plan Section 6.5）：

| 类型 | 含义 | 修复期 |
|------|------|--------|
| Repairable Error (RE) | 字段格式错误 / 缺失必填 | T+5 ET 17:00 |
| Unrepairable Error (UE) | 重复 ID / 时序逻辑不通 | 不可修复，需重新提交完整记录 |
| Late Error (LE) | 超 RE 修复期 | 监管罚款，每条 $1000+ |

#### 2.2.5 拒报与重报流程

```
首次上传 → 等待 Portal 返回 acknowledgment（通常 1-4h）
   ↓
Portal Error Report 解析:
   ├ 无错误 → 完成
   ├ RE 错误 → 自动修复（更新 fdid / 补字段 / 修时间戳）→ 重新生成有问题的 records → 二次上传
   ├ UE 错误 → 人工审查 + 重新提交
   └ 累计 3 次 RE 仍未通过 → P0 告警 + Compliance Officer 介入

CAT 上传连续失败 ≥ 2 天 → 当日交易熔断决策（Compliance Officer 决定）
  此时由 Chief Compliance Officer (CCO) 签字才能继续接收新单
```

#### 2.2.6 NTP / PTP 时钟同步方案

**监管目标**：所有交易事件时间戳精度 ≤ 50ms vs UTC（FINRA Reg SCI Notice 19-22 + CAT NMS Plan §6.7）；CAT 字段精度到微秒。

**方案选型**：

| 方案 | 精度 | 成本 | 部署复杂度 | 适用 |
|------|------|------|----------|------|
| A. AWS Time Sync Service + chrony | < 1ms | $0 | 低 | 全部 trading-engine 节点 |
| B. NTP pool (time.google.com) | 5-50ms | $0 | 低 | 不推荐（精度风险）|
| C. 自建 GPS + PTP IEEE 1588 | < 1μs | 中等（硬件 + 维护）| 高 | Phase 2：FIX 网关 co-location |

**决策**：
- **Phase 1**：方案 A，所有节点强制 chrony 对接 AWS Time Sync（`169.254.169.123`）
- **Phase 2**：FIX 网关在 NY4 / NJ2 co-location 升级 PTP（高频订单需求）
...
**漂移监控**：每节点 node_exporter 暴露 `clock_offset_milliseconds`（来自 `chronyc tracking | grep "System time"`）：

| 阈值 | 级别 | 动作 |
|------|------|------|
| > 10ms | Warning (P2) | Slack 通知，运维查 |
| > 25ms | Critical (P1) | PagerDuty，节点标记不健康 |
| > 50ms | Fatal (P0) | 自动从 LB 摘除该节点，停止接收新订单 |

**证据链**：每个 `order_events` 写入时附带 `clock_offset_us`（写入时刻的本节点漂移）。CAT 生成时校验所有事件 `clock_offset_us < 50000`（即 < 50ms），超阈值的事件标记 RE 并进入 repair queue。

**微秒精度落地**：

```go
// Go 时间戳：保留 nano，输出 μs 精度
func formatCATTimestamp(t time.Time) string {
    return t.UTC().Truncate(time.Microsecond).Format("2006-01-02T15:04:05.000000Z")
}

// DB 列：TIMESTAMP(6) 原生支持 μs；event_data 中也存 nanos 备份
ALTER TABLE order_events
    ALTER COLUMN created_at TYPE TIMESTAMP(6) WITHOUT TIME ZONE,
    ADD COLUMN clock_offset_us INT NOT NULL DEFAULT 0;
```
