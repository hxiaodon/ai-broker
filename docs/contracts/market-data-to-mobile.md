...
|------|------|------|------|-----|---------|
| GET | /api/v1/quotes/:symbol | 单只股票实时报价（最新价、涨跌幅、成交量） | REST | < 200ms (P99) | v1 |
| GET | /api/v1/quotes/batch | 批量报价（自选股列表，最多 50 只） | REST | < 200ms (P99) | v1 |
| GET | /api/v1/kline/:symbol | K 线数据（支持多周期：1min/5min/15min/30min/1h/1D/1W/1M） | REST | < 200ms (P99, Redis); < 500ms (MySQL) | v1 |
| GET | /api/v1/stocks/search | 股票搜索（按名称、代码模糊匹配，支持 US + HK 市场） | REST | < 300ms (P99) | v1 |
| GET | /api/v1/stocks/:symbol | 股票详情：基本面数据、财报摘要、相关新闻 | REST | < 500ms (P99) | v1 |
| GET | /api/v1/watchlist | 自选股列表（用户维度，含最新报价） | REST | < 300ms (P99) | v1 |
| POST | /api/v1/watchlist | 添加自选（symbol + market） | REST | < 200ms (P99) | v1 |
| DELETE | /api/v1/watchlist/:symbol | 删除自选 | REST | < 200ms (P99) | v1 |
| GET | /api/v1/market/status | 市场状态：开盘 / 收盘 / 休市 / 盘前 / 盘后（US + HK） | REST | < 100ms (P99) | v1 |
