...
| 端点 | 方法 | 用途 | SLA | 版本引入 |
|------|------|------|-----|---------|
| /admin/market/status | GET | 各市场交易状态（NYSE/NASDAQ/HKEX 开盘、收盘、休市） | TBD | v1 |
| /admin/market/feeds | GET | 数据源健康检查（各 feed 连接状态、延迟、最后更新时间） | TBD | v1 |
| /admin/stocks | GET | 股票元数据列表（支持按市场/状态筛选、分页） | TBD | v1 |
| /admin/stocks/:symbol | PUT | 股票元数据编辑（停牌标记、交易限制、备注） | TBD | v1 |
