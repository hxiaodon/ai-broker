...
Flutter 使用 `webview_flutter ^4.x` 或 `flutter_inappwebview ^6.x` 实现 WebView，通过以下机制通信：

- **H5 → Native**：JavaScript 调用 `window.FlutterBridge.postMessage(JSON.stringify({method, params}))`
  ...
  onWebViewCreated: (controller) {
    controller.addJavaScriptHandler(
      ...
      callback: (args) {
        ...
        _handleBridgeMessage(message, controller);
      },
    );
  },
...
```dart
void _handleBridgeMessage(Map<String, dynamic> msg, InAppWebViewController ctrl) {
