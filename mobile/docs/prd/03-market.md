...
> **版本**: v2.2
> **日期**: 2026-03-20
> **变更说明**: v2.2 整改 — 采纳 market-data-engineer 技术评审意见（C1/C2/H1/H2/H3/H6/M2）：统一 Watchlist 上限为 100 只；补充交易暂停（HALTED）状态显示规则；明确分时K线仅含常规时段（09:30–16:00）；明确 Phase 1 仅提供 Level 1 报价；补充换手率字段与计算口径；增加搜索 debounce（300ms）和最少字符规则；补充访客延迟行情推送频率（每 5 秒）
...
4. 关键数据网格（今开、昨收、最高、最低、成交量、成交额、市值、**换手率**）
   ...
   > **换手率计算口径**：`换手率 = 当日成交量 ÷ 流通股数 × 100%`，精度 2 位小数；`流通股数` 来自基本面数据，每日收盘后更新
