...
| 入口来源 | 方向预填 | 股票预填 | 数量预填 |
|---------|---------|---------|---------|
| 股票详情页"买入"按钮 | 买入 | 当前股票 | 空，用户填写 |
| 股票详情页"卖出"按钮 | 卖出 | 当前股票 | 空，用户填写 |
| 持仓卡片"买入"按钮 | 买入 | 该持仓股票 | 空，用户填写 |
| 持仓卡片"卖出"按钮 | 卖出 | 该持仓股票 | **预填为该股可卖数量**（已结算股数） |
| 持仓详情页"卖出"按钮 | 卖出 | 该持仓股票 | **预填为该股可卖数量** |

> **设计说明**：从持仓"卖出"发起时预填全部可卖数量，因为此时用户目的明确（清仓或减仓），预填减少操作步骤；从持仓"买入"发起时数量不预填，避免引导用户盲目加仓。
...
**买入/卖出按钮颜色独立于"涨跌颜色偏好"设置**，固定使用以下颜色（面向 UIUX 工程师）：

| 操作 | 按钮颜色 | 说明 |
|------|---------|------|
| 买入 | 绿色（`--color-gain`）| 与港美股盈利语义一致 |
| 卖出 | 红色（`--color-loss`）| 与港美股亏损/减仓语义一致 |

**设计决策原因：**

行情涨跌颜色是"信息展示色"（描述市场状态），买卖按钮颜色是"操作引导色"（引导用户行为）。两者语义层次不同，不应联动。若随用户"红涨绿跌"设置切换，则绿色买入按钮与红色上涨行情数字同时出现，语义矛盾，UIUX 工程师应注意以下处理：

- 行情页（股票详情、持仓卡片）：涨跌颜色跟随用户偏好
- 下单面板、订单确认页的买入/卖出按钮：固定绿买红卖，不随偏好变化
