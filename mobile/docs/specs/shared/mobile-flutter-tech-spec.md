...
| Dart SDK | 3.11.1（随 Flutter 3.41.4 附带） | 开发语言 |
...
Sprint 0 评估完成后，`syncfusion_flutter_charts ^33.1.46` 确认为唯一图表库，`financial_chart` 备选方案不再维护在依赖清单中（见§2.2）。
...
`go_router ^14.8.1` 是 Flutter 官方推荐路由方案，支持声明式路由配置、Deep Link（Universal Link / App Link）、嵌套导航（行情 Tab 内的股票详情嵌套路由），对 WebView 页面的路由封装简洁。
