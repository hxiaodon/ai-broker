...
**Dart 版本**: 3.7.x（随 Flutter 3.41.4 附带）
**文档状态**: 正式采用
...
| 库名 | 版本 | 用途 |
|------|------|------|
| flutter_riverpod | ^2.6.1 | 全局状态管理主框架 |
| riverpod_annotation | ^2.6.1 | 代码生成注解，配合 riverpod_generator |
| riverpod_generator | ^2.6.1 | Riverpod Provider 代码生成（dev dependency） |
| hooks_riverpod | ^2.6.1 | Flutter Hooks + Riverpod 集成（行情页高频更新场景） |
...
| retrofit | ^4.4.1 | 类型安全 REST API 客户端（代码生成，配合 dio） |
| retrofit_generator | ^9.1.3 | retrofit 代码生成（dev dependency） |
| protobuf | ^3.1.0 | Protocol Buffers 序列化（与后端行情数据传输） |
...
| 库名 | 版本 | 用途 |
|------|------|------|
| syncfusion_flutter_charts | ^32.2.9 | K 线图（Candlestick/OHLC）、折线图、面积图，含 pinch-zoom、缩略条；Community License 免费 |
| financial_chart | ^0.4.1 | 备选开源方案（若 Syncfusion 许可有约束时替换） |
...
| image_cropper | ^7.1.5 | 证件图片裁剪（iOS TOCropViewController / Android Ucrop），KYC 四角不遮挡 |
...
| 库名 | 版本 | 用途 |
|------|------|------|
| drift | ^2.23.1 | 类型安全 SQLite ORM，持仓历史、订单缓存、行情快照 |
| drift_dev | ^2.23.1 | drift 代码生成（dev dependency） |
| shared_preferences | ^2.3.4 | 轻量非敏感偏好存储（主题颜色方案、语言设置等） |
...
| 库名 | 版本 | 用途 |
|------|------|------|
| go_router | ^14.6.2 | 声明式路由，Deep Link 处理，WebView 路由 |
| device_info_plus | ^12.3.0 | iOS identifierForVendor、Android ANDROID_ID 等设备信息 |
| uuid | ^4.5.3 | 生成持久化设备 ID（UUID v4）、Idempotency-Key |
| package_info_plus | ^8.1.3 | App 版本号、Build 号（设备指纹、日志标注） |
| intl | ^0.20.2 | 国际化、日期格式化、货币数字格式化 |
| decimal | ^3.2.1 | Dart 高精度十进制数，金融计算专用，替代 double |
| logger | ^2.5.0 | 结构化日志，生产环境 PII 掩码 |
| connectivity_plus | ^6.1.1 | 网络状态检测，离线模式切换 |
| screen_protector | ^1.5.1 | 敏感页面截图防护（FLAG_SECURE + iOS UITextField 方案） |
| flutter_jailbreak_detection | ^1.9.0 | iOS 越狱 / Android Root 检测 |
| permission_handler | ^11.3.1 | 相机、相册、通知权限统一管理 |
...
| 库名 | 版本 | 用途 |
|------|------|------|
| build_runner | ^2.4.14 | 代码生成统一入口（Riverpod、drift、retrofit） |
| flutter_test | SDK 内置 | Widget 单元测试 |
| mocktail | ^1.0.4 | Mock 对象，替代 mockito（无需代码生成） |
| integration_test | SDK 内置 | 集成测试 / UI 端到端测试 |
| flutter_lints | ^5.0.0 | 官方 Lint 规则集 |
| very_good_analysis | ^7.0.0 | 严格 Lint 规则（Very Good Ventures 推荐，适合金融 App） |
...
选择 Riverpod 而非 BLoC 的主要原因：金融 App 中有大量跨 Widget 共享的全局状态（行情价格、账户余额、订单状态），Riverpod 的 Provider 天然支持跨 Widget 树访问且不依赖 `BuildContext`；`AsyncNotifierProvider` 对 `AsyncValue<T>` 的三态（loading/data/error）处理比 BLoC 的 `emit(LoadingState())` 更简洁，减少金融数据加载场景的样板代码。Riverpod 2.x 的代码生成（`@riverpod` 注解 + riverpod_generator）消除了手动定义 Provider 类型的错误。Provider 包已进入维护模式，不推荐新项目使用。

**图表：Syncfusion 优先，financial_chart 备选**

`syncfusion_flutter_charts v32.2.9` 是商业级方案，在 iOS 上经过大量生产验证，Candlestick 图表支持完整（含上下影线、OHLC、缩略条、跨图表 trackball），pinch-zoom 手势与 Impeller 渲染引擎的集成经验丰富。Community License 对年营收 100 万美元以下的公司免费，超过此阈值后需商业授权。Sprint 0 需同时测试 `financial_chart v0.4.1`，若 Syncfusion 授权产生障碍则切换至后者。
...
│   │   │   ├── remote/
│   │   │   │   └── auth_api.dart       # retrofit 生成的 API 接口
│   │   │   └── local/
...
`image_picker v1.2.1` 选取，`image_cropper v7.1.5` 裁剪：
...
class SecureStorageService {
  static const _storage = FlutterSecureStorage(
    ...
    aOptions: AndroidOptions(
      // 使用 EncryptedSharedPreferences（Android Keystore 保护密钥）
...
class JailbreakDetectionService {
  Future<SecurityCheckResult> check() async {
    ...
    if (isDeveloperMode && kReleaseMode) {
      return SecurityCheckResult.developerModeEnabled;
    }
...
- `jailbroken`：显示警告对话框，禁止交易功能（仍允许查看行情），不允许关闭对话框
- `developerModeEnabled`：仅在 Release 构建时警告，Debug 构建忽略（方便开发）
- `clean`：正常运行

### 6.4 证书固定（Certificate Pinning）

通过 dio 的 `onHttpClientCreate` 回调配置证书固定：

```dart
// core/security/ssl_pinning_config.dart
class SslPinningConfig {
  // 证书 SHA-256 指纹（生产环境主证书 + 备用证书，备用于证书轮换时过渡）
  ...
  static SecurityContext createSecurityContext() {
    ...
    // 加载预置的证书
    for (final certAsset in ['assets/certs/api.crt']) {
      final certBytes = _loadCertBytes(certAsset);
...
Dio createDioClient() {
  ...
  (dio.httpClientAdapter as IOHttpClientAdapter).createHttpClient = () {
    final client = HttpClient(context: SslPinningConfig.createSecurityContext());
    client.badCertificateCallback = (cert, host, port) {
      // 验证证书指纹
      final fingerprint = _computeSha256Fingerprint(cert.der);
      return SslPinningConfig.pinnedFingerprints.contains(fingerprint);
    };
    return client;
  };
...
证书轮换说明：轮换前至少 2 周将新证书指纹加入备用列表（`_pinnedCertificates`）并发版，轮换后等待旧版本用户更新，再移除旧指纹，发版清除。
