...
| 库名 | 版本 | 用途 |
|------|------|------|
| flutter_riverpod | ^3.0.0 | 全局状态管理主框架（Riverpod 3.0 稳定版，2025-09，含自动重试/暂停恢复） |
| riverpod_annotation | ^3.0.0 | 代码生成注解，配合 riverpod_generator（Riverpod 3.0 稳定版，2025-09，含自动重试/暂停恢复） |
| riverpod_generator | ^3.0.0 | Riverpod Provider 代码生成（dev dependency；Riverpod 3.0 稳定版，2025-09，含自动重试/暂停恢复） |
| hooks_riverpod | ^3.0.0 | Flutter Hooks + Riverpod 集成（行情页高频更新场景；Riverpod 3.0 稳定版，2025-09，含自动重试/暂停恢复） |
...
| http_certificate_pinning | ^1.0.2 | SPKI 公钥指纹比对，配合 dio 实现证书固定 |
...
| image_cropper | ^11.0.0 | 证件图片裁剪（iOS TOCropViewController / Android Ucrop），KYC 四角不遮挡 |
...
| 库名 | 版本 | 用途 |
|------|------|------|
| go_router | ^14.6.2 | 声明式路由，Deep Link 处理，WebView 路由（v14.x onExit 回调签名变更，见§3.4） |
...
| 库名 | 版本 | 用途 |
|------|------|------|
| build_runner | ^2.4.14 | 代码生成统一入口（Riverpod、drift） |
...
| very_good_analysis | ^10.1.0 | 严格 Lint 规则（Very Good Ventures 推荐，适合金融 App；Dart 3.11 兼容） |
...
选择 Riverpod 而非 BLoC 的主要原因：金融 App 中有大量跨 Widget 共享的全局状态（行情价格、账户余额、订单状态），Riverpod 的 Provider 天然支持跨 Widget 树访问且不依赖 `BuildContext`；`AsyncNotifierProvider` 对 `AsyncValue<T>` 的三态（loading/data/error）处理比 BLoC 的 `emit(LoadingState())` 更简洁，减少金融数据加载场景的样板代码。本项目使用 **Riverpod 3.0**（2025-09 正式版）：
...
**网络层：手写 Data Source 而非 retrofit 代码生成**
